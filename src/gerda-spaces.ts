//
// Gerda - The optimized Arendelle intelligent auto suggestions server
//    Copyright 2016 Kary Foundation, Inc.
//    Author: Pouya Kary <k@karyfoundation.org>
//

export module Gerda.Kernel {
	...
	// ────────────────────────────────────────────────────────────────────────────────────────────────────
		...
		export function GetSpaces ( blueprintText: string , caretLocation: number ): Array<string> {
			...
			//
				...
				/** To keep track of the characters location within the blueprint */
			...
			//
				...
				/**
		 		 * This checks on the list to not to add a space that is already declared.
				 */
				var CheckForDuplicateAndAdd = function ( spaceToCheck: IScopedSpaceWithKey ) {
				...
				/**
				 * Adds a space name to the scoped space list if the space name
				 * meets the space name regex and also is not already added.
				 */
				var AppendSpace = function ( givenSpaceName: string ) {
					...
					if ( givenSpaceName.match( spaceNameRegexRule ) ) {
						...
						CheckForDuplicateAndAdd( [ scopeLevel , nameKey , givenSpaceName ] );
					}
				}
			...
			//
				...
				for ( scopeLevel = 0 ; characterLocationIndex < whereToStop ; characterLocationIndex++ ) {
					...
					var ReadAndParseHeader = function ( ) {
						...
						var possibleSpaceDecelerations =	headerText.split( ',' );
						possibleSpaceDecelerations.forEach( possibleSpaceName => {
					...
					if ( currentChar == ' ' || currentChar == '\t' || currentChar == '\n' ) {
						 ...
						 * the scanning for normal scope decelerations
						 */
						break;
					}
				}
			...
			//
				...
				for ( scopeLevel = 1 ; characterLocationIndex < whereToStop ; characterLocationIndex++ ) {
					...
					// If there's an space deceleration...
					if ( currentChar == '(' ) {
						var finding: string = '';
						NextCharacter( );
